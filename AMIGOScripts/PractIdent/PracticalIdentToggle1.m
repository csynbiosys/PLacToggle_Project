...
% MODEL RELATED DATA
%============================
inputs.model = ToggleSwitch_load_model_M1SF_Iter0();
inputs.model.par(17:18) = [1.5, 1.5];                                            % These values may be updated during optimization
...
%==================================
AMIGO_Prep(inputs); 
y0 = M1SF_Compute_SteadyState_OverNight(inputs,inputs.model.par,[26, 1300],[1, 0]+1e-7);

inputs.exps.n_exp=5;                                 % Number of experiments
for i = 1:5
    inputs.exps.n_obs{i}=2;                              % Number of observed quantities per experiment
    inputs.exps.obs_names{i}=char('LacI_M1','TetR_M1');          % Observables
    inputs.exps.obs{i}=char('LacI_M1 = L_RFP_AU','TetR_M1 = T_GFP_AU'); % Observation function
...
for i = 1:5
 ...
 inputs.PEsol.id_global_theta='all';                  % 'all'|User selected
   ...
   inputs.PEsol.global_theta_max= [0.4488,0.4488,...
                              1.1220,1.1220,...
    ...
    inputs.PEsol.global_theta_min= [0.0036,0.0036,...
              0.0089,0.0089,...
...
save("PracticalIdentifiability_Model1.mat", "PIdent")
